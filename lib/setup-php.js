...
const isComposer = core.getInput('composer');
...
async function run() {
  ...
  const NETWORK = await new Promise((resolve, reject) => {
    ...
    const cmd = '/usr/bin/docker network ls --filter name=github -q | head -1';
  ...
  const IMAGE = `khs1994/php:${PHP_VERSION}-${TYPE}-alpine`;
